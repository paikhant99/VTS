...
class UserRepositoryImpl extends UserRepository{
  ...
  UserRepositoryImpl.namedPrivate({required ApiService apiService, required UserPreferences userPreferences}) : _apiService = ApiService(), _userPreferences = UserPreferences();
  
  @override
  Future<Result<LoginResponse, Exception>> loginUser(String username, String password) async{
    http.Response response = await _apiService.login(LoginRequest(username: username, password: password));
    if (response.statusCode == HttpStatus.ok){
      final loginResponse = LoginResponse.fromJson(jsonDecode(response.body));
      _userPreferences.saveLoginInfo(loginResponse.userId, loginResponse.username, loginResponse.dateOfBirth, loginResponse.gender, loginResponse.access);
  ...
  @override
  Future<Result<RegisterResponse, Exception>> registerPatient(String username, String email, String password, String dateOfBirth, Gender gender, String? address) async{
    http.Response response = await _apiService.register(RegisterRequest(username: username, email: email, password: password, dateOfBirth: dateOfBirth, gender: gender.name, address: address));
